...
<stripes:layout-render name="/WEB-INF/jsp/layout/default.jsp">
    <stripes:layout-component name="title">
        <c:choose>
            <c:when test="${actionBean.complete}">
                Klaar - (${actionBean.proces.id}) transformeren
            </c:when>
            <c:otherwise>
                [<fmt:formatNumber maxFractionDigits="1" value="${actionBean.progress}"/>%] Bezig met transformeren ${actionBean.proces.soort} (${actionBean.proces.id})...
            </c:otherwise>
        </c:choose>
    ...
    <stripes:layout-component name="contents">
        <h1>Laadprocessen ${actionBean.proces.soort} (${actionBean.proces.id}) transformeren</h1>
        <p>
