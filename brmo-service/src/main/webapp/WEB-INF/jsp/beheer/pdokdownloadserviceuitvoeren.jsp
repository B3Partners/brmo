...
<stripes:layout-render name="/WEB-INF/jsp/layout/default.jsp">
    <stripes:layout-component name="title">
        <c:choose>
            <c:when test="${actionBean.complete}">
                Klaar - PDOK Download Service (${actionBean.proces.id})
            </c:when>
            <c:otherwise>
                [<fmt:formatNumber maxFractionDigits="1" value="${actionBean.progress}"/>%] Bezig met uitvoeren PDOK Download Service (${actionBean.proces.id})...
            </c:otherwise>
        </c:choose>
    ...
    <stripes:layout-component name="contents">
        <h1>PDOK Download Service (${actionBean.proces.id}) ophalen</h1>
        <p>
