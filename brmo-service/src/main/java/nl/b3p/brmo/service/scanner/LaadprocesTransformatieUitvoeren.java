...
public class LaadprocesTransformatieUitvoeren extends AbstractExecutableProces {
    ...
    public void execute(final ProgressUpdateListener listener) {
        ...
        String msg =
                String.format(
                        "Het laadproces transformatie proces met ID %d is gestart op %tc.",
        ...
        try {
            ...
            if (LaadprocesTransformatieProces.LaadprocesSoorten.BR_TOPNL
                    .getSoort()
                    .equalsIgnoreCase(this.config.getSoort())) {
            ...
            if (LaadprocesTransformatieProces.LaadprocesSoorten.BR_TOPNL
                    ...
                    ids.addAll(
                            Arrays.asList(
                                    brmo.getLaadProcessenIds(
                                            "bestand_datum", "ASC", type, "STAGING_OK")));
                }
                ...
                lpIds =
                        ArrayUtils.toPrimitive(
                                brmo.getLaadProcessenIds(
                                        "bestand_datum", "ASC", config.getSoort(), "STAGING_OK"));
            }
            brmo.setOrderBerichten(!config.alsStandTransformeren());
            if (lpIds == null || lpIds.length == 0) {
                msg =
                        "Er zijn geen laadprocessen van soort "
                                + config.getSoort()
                                + " en status: 'STAGING_OK' om te transformeren.";
                LOG.info(msg);
                sb.append(msg).append(AutomatischProces.LOG_NEWLINE);
                listener.addLog(msg);
            } else {
            ...
            if (transformErrorOccured) {
                ...
                msg =
                        String.format(
                                "Laadproces transformatie proces met ID %d is niet succesvol afgerond op %tc.",
                                config.getId(), Calendar.getInstance());
                LOG.error(msg);
                ...
                msg =
                        String.format(
                                "Laadproces transformatie proces met ID %d is succesvol afgerond op %tc.",
                                config.getId(), Calendar.getInstance());
                LOG.info(msg);
            ...
            String m =
                    "Fout bij transformeren laadprocessen naar RSGB: "
