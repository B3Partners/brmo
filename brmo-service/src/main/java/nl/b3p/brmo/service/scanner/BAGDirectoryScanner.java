...
public class BAGDirectoryScanner extends AbstractExecutableProces {
  ...
  public void execute(ProgressUpdateListener listener) {
    ...
    String msg =
        String.format(
            "De BAG scanner met ID %d is gestart op %tc.", config.getId(), Calendar.getInstance());
    LOG.info(msg);
    ...
    if (isArchiving) {
      ...
      if (!archiefDirectory.isDirectory() || !archiefDirectory.canWrite()) {
        ...
        msg =
            String.format(
                "De archief directory '%s' is geen beschrijfbare directory", archiefDirectory);
        LOG.error(msg);
        listener.updateStatus(msg);
        listener.addLog(msg);
        this.listener.exception(new BrmoException(msg));
      ...
      if (!scanDirectory.canWrite()) {
        ...
        msg =
            String.format(
                String.format(
                    "De scan directory '%s' is geen beschrijfbare directory", scanDirectory));
        LOG.error(msg);
        listener.updateStatus(msg);
        listener.addLog(msg);
        this.listener.exception(new BrmoException(msg));
    ...
    msg =
        String.format(
            "De BAG scanner met ID %d is afgerond op %tc.", config.getId(), Calendar.getInstance());
    LOG.info(msg);
    listener.updateStatus(msg);
    listener.addLog(msg);
    sb.append(msg).append(AutomatischProces.LOG_NEWLINE);
