...
--  f.geom AS wpl_geom,
   ...
   FROM nummeraand n 
     ...
     JOIN openb_rmte b ON a.fk_7opr_identifcode = b.identifcode
     JOIN openb_rmte_gem_openb_rmte c ON b.identifcode = c.fk_nn_lh_opr_identifcode
     JOIN gem_openb_rmte d ON c.fk_nn_rh_gor_identifcode = d.identifcode
     JOIN openb_rmte_wnplts e ON b.identifcode = e.fk_nn_lh_opr_identifcode
     JOIN wnplts f ON e.fk_nn_rh_wpl_identif = f.identif
...
--    b.vlakgeom,
    b.puntgeom
   FROM verblijfsobj a
     JOIN gebouwd_obj b ON a.sc_identif = b.sc_identif
...
create or replace view pv_natuurlijk_persoon as
 SELECT a.sc_identif,
    ...
    b.telefoonnummer
   FROM prs c
     JOIN subject b ON c.sc_identif = b.identif
     JOIN nat_prs a ON a.sc_identif = c.sc_identif;
...
select a.sc_identif,
 ...
 from prs c
    JOIN subject b ON c.sc_identif = b.identif
    JOIN niet_nat_prs a ON a.sc_identif = c.sc_identif
    LEFT JOIN ingeschr_niet_nat_prs d ON d.sc_identif = a.sc_identif;
...
**************************************************/
comment on table pv_adr_object_nummeraand is 'aliased as aon, uses addresseerb_obj, openb_rmte, openb_rmte_gem_openb_rmte, gem_openb_rmte, wnplts, gemeente';
comment on table pv_ander_natuurlijk_persoon is 'aliased as anp, uses ander_nat_prs';
comment on table pv_appartementsrecht is 'aliased as apr, uses pv_appartementsrecht';
comment on table pv_benoemd_obj_kad_onr_zk is 'aliased as bok, uses benoemd_obj_kad_onrrnd_zk';
comment on table pv_benoemd_object is 'aliased as bob, uses benoemd_obj';
comment on table pv_benoemd_terrein is 'aliased as bte, uses benoemd_terrein';
comment on table pv_gebouwd_obj_gebruiksdoel is 'aliased as gog, uses gebouwd_obj_gebruiksdoel';
comment on table pv_gebouwd_object is 'aliased as gob, uses gebouwd_obj';
comment on table pv_gemeente is 'aliased as gem, uses gemeente';
comment on table pv_ingeschr_natuurlijk_persoon IS 'aliased as inp, uses ingeschr_nat_prs';
comment on table pv_kad_onr_zk_aantek is 'aliased as koa, uses kad_onrrnd_zk_aantek';
comment on table pv_kad_onr_zk_his_rel is 'aliased as koh, uses kad_onrrnd_zk_his_rel';
comment on table pv_kad_onroerende_zaak is 'aliased as koz, uses kad_onrrnd_zk';
comment on table pv_kad_perceel is 'aliased as kpe, uses kad_perceel, kad_onrrnd_zk';
comment on table pv_ligplaats is 'aliased as lip, uses ligplaats, benoemd_terrein';
comment on table pv_ligplaats_nummeraand is 'aliased as lin, uses ligplaats_nummeraand';
comment on table pv_natuurlijk_persoon is 'aliased as npe, uses prs, subject, nat_prs';
comment on table pv_niet_ingezetene is 'aliased as nig, uses niet_ingezetene';
comment on table pv_niet_natuurlijk_persoon is 'aliased as nnp, uses subject, niet_nat_prs, ingeschr_niet_nat_prs';
comment on table pv_pand is 'aliased as pnd, uses pand';
comment on table pv_persoon is 'aliased as pso, uses prs';
comment on table pv_standplaats is 'aliased as stp, uses standplaats, benoemd_terrein';
comment on table pv_standplaats_nummeraand is 'aliased as stn, uses standplaats_nummeraand';
comment on table pv_verblijfsobj_nummeraand is 'aliased as von, uses verblijfsobj_nummeraand';
comment on table pv_verblijfsobj_pand is 'aliased as vop, uses verblijfsobj_pand';
comment on table pv_verblijfsobject is 'aliased as vob, uses verblijfsobj, gebouwd_obj';
comment on table pv_woonplaats is 'aliased as wpl, uses wnplts, gemeente';
comment on table pv_zakelijk_recht is 'aliased as zre, uses zak_recht, aard_recht_verkort';
comment on table pv_zakelijk_recht_aantekening is 'aliased as zra, uses zak_recht_aantek';
