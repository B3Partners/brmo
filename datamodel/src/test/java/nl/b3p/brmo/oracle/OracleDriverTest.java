...
public class OracleDriverTest {
  ...
  public void testOracleDriverImpl() throws SQLException {
    ...
    if (isOracle) {
      ...
      String prepSQL =
          "select * from (select /*+ FIRST_ROWS(n) */ a.*, ROWNUM rnum from (select * from bericht where job_id = ? order by soort, object_ref, datum, volgordenummer ) a where ROWNUM <=250 ) where rnum  > 0";
