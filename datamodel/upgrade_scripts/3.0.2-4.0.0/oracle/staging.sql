...
BEGIN
    ...
    EXECUTE IMMEDIATE 'CREATE PRIVATE TEMPORARY TABLE ora$ptt_pids AS (SELECT apc.proces_id FROM automatisch_proces_config apc WHERE to_char(apc.value) = ''bag'' AND apc.config_key=''gds2_br_soort'')';
    EXECUTE IMMEDIATE 'SELECT * FROM ora$ptt_pids';
    EXECUTE IMMEDIATE 'DELETE FROM automatisch_proces_config WHERE proces_id IN (SELECT proces_id FROM ora$ptt_pids)';
    EXECUTE IMMEDIATE 'DELETE FROM automatisch_proces WHERE id IN (SELECT proces_id FROM ora$ptt_pids)';
    EXECUTE IMMEDIATE 'DROP TABLE ora$ptt_pids';
END;
