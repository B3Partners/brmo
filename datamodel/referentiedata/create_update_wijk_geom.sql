...
--
-- 1. laadt de wijk kaart uit de wijk en buurt kaart van CBS (buurt_2018.zip) in een postgis database
-- ogr2ogr -f "PostgreSQL" "PG:dbname=bestuurlijkegrenzen port=5434 host=localhost user=mark password=mark" shape2018versie10/wijk_2018.shp -append -nln public.wijk2018 -select "wk_code, water" -nlt PROMOTE_TO_MULTI -a_srs "EPSG:28992"
-- of gebruik shp2pgsql of shp2pgsql-gui
-- vereenvoudig de geometrie met "update wijk2018 set geom = ST_SnapToGrid(geom, 0.01);" (afronden cijfers achter de komma op 1cm
--
