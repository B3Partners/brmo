...
create or replace view pv_map_o_kpe as
 SELECT a.cu_aard_bebouwing,
    ...
    (a.ka_sectie || ' ') || a.ka_perceelnummer AS aanduiding,
...
as
 SELECT koz.kad_identif AS koz_identif,
    ...
    CASE
        WHEN npe.sc_identif IS NOT NULL THEN 'Natuurlijk persoon'
   ...
   FROM pm_kad_onroerende_zaak koz
     ...
     LEFT JOIN pm_persoon pso on  pso.sc_identif = zre.fk_8pes_sc_identif
     LEFT JOIN pm_natuurlijk_persoon npe ON npe.sc_identif = pso.sc_identif
     LEFT JOIN pm_ingeschr_natuurlijk_persoon inp ON inp.sc_identif = pso.sc_identif
     LEFT JOIN pm_niet_natuurlijk_persoon nnp ON nnp.sc_identif =  pso.sc_identif
