...
***************************************************/
create or replace view pv_gemeente as
...
***************************************************/
create or replace view pv_woonplaats as
 SELECT a.dat_beg_geldh,
        a.identif,
        a.datum_einde_geldh,
--		a.indic_geconst,
...
--      b.naam_nen gem_naam_nen
    FROM wnplts a
     LEFT JOIN gemeente b ON a.fk_7gem_code = b.code;
...
************************************************************/                                                              
create or replace view pv_adr_object_nummeraand as                                                                          
 SELECT     a.identif,
            a.dat_beg_geldh,
    a.dat_eind_geldh,
    a.huinummer,
...
--  f.geom AS wpl_geom,
    ...
    n.status
   FROM nummeraand n 
     JOIN addresseerb_obj_aand a on n.sc_identif = a.identif
     JOIN openb_rmte b ON a.fk_7opr_identifcode::text = b.identifcode::text
     JOIN openb_rmte_gem_openb_rmte c ON b.identifcode::text = c.fk_nn_lh_opr_identifcode::text
     JOIN gem_openb_rmte d ON c.fk_nn_rh_gor_identifcode::text = d.identifcode::text
     JOIN openb_rmte_wnplts e ON b.identifcode::text = e.fk_nn_lh_opr_identifcode::text
     JOIN wnplts f ON e.fk_nn_rh_wpl_identif::text = f.identif::text
     JOIN gemeente g ON f.fk_7gem_code = g.code;
...
***************************************************/
create or replace view pv_pand as
 SELECT 
    a.dat_beg_geldh,
    a.identif,
    --a.bruto_inhoud,
...
create or replace view pv_verblijfsobject as
 SELECT 
    a.sc_identif,
--    a.aantal_kamers,
...
--    b.vlakgeom,
    b.puntgeom
   FROM verblijfsobj a
     JOIN gebouwd_obj b ON a.sc_identif::text = b.sc_identif::text
...
***************************************************/
create or replace view pv_verblijfsobj_pand as
...
***************************************************/
create or replace view pv_verblijfsobj_nummeraand as
 SELECT a.fk_nn_lh_vbo_sc_identif,
        a.fk_nn_lh_vbo_sc_dat_beg_geldh,
...
***************************************************/
create or replace view pv_gebouwd_obj_gebruiksdoel as
...
***************************************************/
create or replace view pv_benoemd_object as
SELECT  a.identif,
        a.clazz
   FROM benoemd_obj a
;
...
***************************************************/
create or replace view pv_gebouwd_object as
SELECT  a.dat_beg_geldh,
        a.sc_identif,
        a.clazz,
-- 	a.bouwk_best_act,
...
OPMERKKING 27-3-2015: JOIN MET BENOEMD_OBJ IS VERVALLEN, GEEN MEERWAARDE
***************************************************/
create or replace view pv_benoemd_terrein as
 SELECT  a.dat_beg_geldh,
         a.sc_identif,
		 a.clazz,
         a.datum_einde_geldh,
...
***************************************************/
create or replace view pv_standplaats as
 SELECT a.sc_identif,
        a.indic_geconst,
        a.status,
        ...
        b.geom AS bt_geom
   FROM standplaats a
     JOIN benoemd_terrein b ON a.sc_identif::text = b.sc_identif::text;
...
***************************************************/
create or replace view pv_standplaats_nummeraand as
...
***************************************************/
create or replace view pv_ligplaats as
 SELECT a.sc_identif,
	    a.indic_geconst,
        a.status,
        a.fk_4nra_sc_identif,
        ...
        b.geom AS bt_geom
	FROM ligplaats a
   JOIN benoemd_terrein b ON a.sc_identif = b.sc_identif;
...
***************************************************/
create or replace view pv_ligplaats_nummeraand as
...
***************************************************/  
create or replace view pv_kad_perceel as
...
--    a.typering,
   FROM kad_perceel b
     JOIN kad_onrrnd_zk a ON b.sc_kad_identif = a.kad_identif;
...
***************************************************/  
create or replace view pv_kad_onroerende_zaak as
 SELECT 
--  a.cu_aard_bebouwing,
    a.cu_aard_cultuur_onbebouwd,
--    a.cu_meer_culturen,
...
***************************************************/  
create or replace view pv_kad_onr_zk_his_rel as
 SELECT 
    a.fk_sc_lh_koz_kad_identif,
...
***************************************************/  
create or replace view pv_kad_onr_zk_aantek as
SELECT 
    a.begindatum_aantek_kad_obj,
...
***************************************************/  
create or replace view pv_zakelijk_recht as
 SELECT a.kadaster_identif,
	...
	a.fk_3avr_aand
   FROM zak_recht a
   JOIN aard_recht_verkort ar ON ar.aand = a.fk_3avr_aand;
...
***************************************************/ 
create or replace view pv_zakelijk_recht_aantekening as
...
***************************************************/ 
create or replace view pv_appartementsrecht as
...
--	a.typering
   FROM kad_onrrnd_zk a
     JOIN app_re b ON a.kad_identif = b.sc_kad_identif;
...
***************************************************/ 
create or replace view pv_persoon as
 SELECT a.sc_identif,
        a.clazz
   FROM prs a
;
...
***************************************************/ 
create or replace view pv_natuurlijk_persoon as
 SELECT a.sc_identif,
        a.clazz,
		a.aand_naamgebruik,
        a.geslachtsaand,
    ...
    b.telefoonnummer
   FROM prs c
     JOIN subject b ON c.sc_identif::text = b.identif::text
     JOIN nat_prs a ON a.sc_identif::text = c.sc_identif::text;
...
***************************************************/ 
create or replace view pv_ingeschr_natuurlijk_persoon as          
select sc_identif,
            gb_geboortedatum, 
            gb_geboorteplaats,
            ol_overlijdensdatum,
...
***************************************************/ 
create or replace view pv_niet_ingezetene as          
select sc_identif
from niet_ingezetene;
...
***************************************************/ 
create or replace view pv_ander_natuurlijk_persoon as
select sc_identif, 
       geboortedatum, 
	   overlijdensdatum
from   ander_nat_prs;
...
***************************************************/ 
create or replace view pv_niet_natuurlijk_persoon as
select a.sc_identif,
       a.clazz,
       a.naam AS nnp_naam,
       -- a.datum_aanvang,
 ...
 from prs c
    JOIN subject b ON c.sc_identif::text = b.identif::text
    JOIN niet_nat_prs a ON a.sc_identif::text = c.sc_identif::text
    LEFT JOIN ingeschr_niet_nat_prs d ON d.sc_identif::text = a.sc_identif::text;
...
***************************************************/
create or replace view pv_benoemd_obj_kad_onr_zk as
SELECT  a.fk_nn_lh_tgo_identif::numeric::text fk_nn_lh_tgo_identif,
...
**************************************************/
comment on view pv_adr_object_nummeraand is 'aliased as aon, uses addresseerb_obj, openb_rmte, openb_rmte_gem_openb_rmte, gem_openb_rmte, wnplts, gemeente';
comment on view pv_ander_natuurlijk_persoon is 'aliased as anp, uses ander_nat_prs';
comment on view pv_appartementsrecht is 'aliased as apr, uses pv_appartementsrecht';
comment on view pv_benoemd_obj_kad_onr_zk is 'aliased as bok, uses benoemd_obj_kad_onrrnd_zk';
comment on view pv_benoemd_object is 'aliased as bob, uses benoemd_obj';
comment on view pv_benoemd_terrein is 'aliased as bte, uses benoemd_terrein';
comment on view pv_gebouwd_obj_gebruiksdoel is 'aliased as gog, uses gebouwd_obj_gebruiksdoel';
comment on view pv_gebouwd_object is 'aliased as gob, uses gebouwd_obj';
comment on view pv_gemeente is 'aliased as gem, uses gemeente';
comment on view pv_ingeschr_natuurlijk_persoon IS 'aliased as inp, uses ingeschr_nat_prs';
comment on view pv_kad_onr_zk_aantek is 'aliased as koa, uses kad_onrrnd_zk_aantek';
comment on view pv_kad_onr_zk_his_rel is 'aliased as koh, uses kad_onrrnd_zk_his_rel';
comment on view pv_kad_onroerende_zaak is 'aliased as koz, uses kad_onrrnd_zk';
comment on view pv_kad_perceel is 'aliased as kpe, uses kad_perceel, kad_onrrnd_zk';
comment on view pv_ligplaats is 'aliased as lip, uses ligplaats, benoemd_terrein';
comment on view pv_ligplaats_nummeraand is 'aliased as lin, uses ligplaats_nummeraand';
comment on view pv_natuurlijk_persoon is 'aliased as npe, uses prs, subject, nat_prs';
comment on view pv_niet_ingezetene is 'aliased as nig, uses niet_ingezetene';
comment on view pv_niet_natuurlijk_persoon is 'aliased as nnp, uses subject, niet_nat_prs, ingeschr_niet_nat_prs';
comment on view pv_pand is 'aliased as pnd, uses pand';
comment on view pv_persoon is 'aliased as pso, uses prs';
comment on view pv_standplaats is 'aliased as stp, uses standplaats, benoemd_terrein';
comment on view pv_standplaats_nummeraand is 'aliased as stn, uses standplaats_nummeraand';
comment on view pv_verblijfsobj_nummeraand is 'aliased as von, uses verblijfsobj_nummeraand';
comment on view pv_verblijfsobj_pand is 'aliased as vop, uses verblijfsobj_pand';
comment on view pv_verblijfsobject is 'aliased as vob, uses verblijfsobj, gebouwd_obj';
comment on view pv_woonplaats is 'aliased as wpl, uses wnplts, gemeente';
comment on view pv_zakelijk_recht is 'aliased as zre, uses zak_recht, aard_recht_verkort';
comment on view pv_zakelijk_recht_aantekening is 'aliased as zra, uses zak_recht_aantek';
