...
class Brk2StandMutatieVerwerkingIntegrationTest extends AbstractDatabaseIntegrationTest {
    ...
    void testBericht(
        ...
        // stand bericht laden
        this.bestandLadenEnValideren(objectRef, bestandNaam);
        ...
        this.bestandLadenEnValideren(objectRef, mutatieBestandNamen);
...
//        ITable adres = rsgbBrk.createDataSet().getTable("adres");
     ...
     * @throws Exception if any
     */
    private void bestandLadenEnValideren(String objectRef, String... bestandsNamen) throws Exception {
        int aantal = bestandsNamen.length;
        String soort;
        if (bestandsNamen[0].contains("stand")) {
            soort = "stand";
        } else {
            soort = "mutatie";
            aantal += 1/*stand bericht*/;
        }

        for (String bestandsNaam : bestandsNamen) {
            LOG.debug("Laden van " + soort + "bericht in staging DB.");
