...
public class StagingProxy {
    ...
    public Bericht getExistingBericht(Bericht b) throws SQLException {
        return getBerichtByNaturalKey(
                b.getObjectRef(), b.getDatum().getTime(), b.getVolgordeNummer());
    }
    ...
    public void handleBerichtenByJob(
        ...
        final ProcessDbXmlPipeline processDbXmlPipeline =
                new ProcessDbXmlPipeline(
                        handler,
                        transformPipelineCapacity,
                        "b3p.rsgb.job." + dateTime + ".pipeline");
        if (enablePipeline) {
        ...
        try {
            do {
                ...
                final Split getBerichten =
                        SimonManager.getStopwatch(
                                        "b3p.rsgb.job." + dateTime + ".staging.berichten.getbatch")
                                .start();
                Exception e =
                        new QueryRunner(geomToJdbc.isPmdKnownBroken())
                                .query(
                                        ...
                                        rs -> {
                                            ...
                                            while (rs.next()) {
                                                try {
                                                    Bericht bericht =
                                                            processor.toBean(rs, Bericht.class);
                                                    if (bericht.getVolgordeNummer() >= 0
                                                            && !doOrderBerichten) {
                                                    ...
                                                    final BerichtWorkUnit workUnit =
                                                            new BerichtWorkUnit(bericht);

                                                    if (enablePipeline) {
                                                        List<TableData> tableData =
                                                                handler.transformToTableData(
                                                                        bericht);
                                                        if (tableData == null) {
                                                        ...
                                                        workUnit.setTypeOfWork(
                                                                BerichtTypeOfWork.PROCESS_DBXML);
                                                        Split pipelinePut =
                                                                SimonManager.getStopwatch(
                                                                                "b3p.rsgb.job."
                                                                                        + dateTime
                                                                                        + ".pipeline.processdbxml.put")
                                                                        .start();
                                                        processDbXmlPipeline
                                                                .getQueue()
                                                                .put(workUnit);
                                                        pipelinePut.stop();
                                                    } else {
            ...
            if (offset < total) {
                log.warn(
                        String.format(
                                "Minder berichten verwerkt (%d) dan verwacht (%d)!",
                                offset, total));
            }
    ...
    public void deleteByLaadProcesId(Long id) throws SQLException {
        ...
        new QueryRunner(geomToJdbc.isPmdKnownBroken())
                .update(
                        getConnection(),
                        "DELETE FROM " + BrmoFramework.LAADPROCES_TABEL + " WHERE id = ?",
                        id);
    }
    ...
    public List<LaadProces> getLaadProcessen() throws SQLException {
        ...
        list =
                new QueryRunner(geomToJdbc.isPmdKnownBroken())
                        .query(
                                getConnection(),
                                "select * from " + BrmoFramework.LAADPROCES_TABEL,
                                h);
        return list;
    }
    ...
    public void loadBr(
        ...
        if (laadProcesExists(lp)) {
            throw new BrmoDuplicaatLaadprocesException(
                    "Laadproces al gerund voor bestand " + fileName);
        }
        ...
        if (TopNLType.isTopNLType(type)) {
            ...
            if (!brmoXMLReader.hasNext()) {
                ...
                throw new BrmoLeegBestandException(
                        "Leeg bestand, geen berichten gevonden in " + fileName);
            }
            ...
            while (brmoXMLReader.hasNext()) {
                ...
                try {
                    ...
                    if (type.equals(BrmoFramework.BR_BRK) && !isBerichtGeschreven) {
                        ...
                        lp.setBestandNaamHersteld(
                                brkBericht.getRestoredFileName(
                                        lp.getBestandDatum(), b.getVolgordeNummer()));
                    ...
                    if (type.equals(BrmoFramework.BR_BRK2) && !isBerichtGeschreven) {
                        ...
                        lp.setBestandNaamHersteld(
                                brk2Bericht.getRestoredFileName(
                                        lp.getBestandDatum(), b.getVolgordeNummer()));
                        updateLaadProcesBestandNaamHersteld(lp);
                    }
                    ...
                    lastErrorMessage =
                            String.format(
                                    "Laden bericht uit %s mislukt vanwege: %s",
