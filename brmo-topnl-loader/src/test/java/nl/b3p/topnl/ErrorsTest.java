...
public class ErrorsTest extends TestUtil {
    ...
    @Test
    public void polygonErrors1Test()
        ...
        Terrein real =
                run.query(
                        "SELECT * FROM top250nl.terrein WHERE identificatie=?",
                        handler,
                        "NL.TOP250NL.16R09-0000064537");
        assertNotNull(real);
    }

    @Test
    public void polygonErrors2Test()
            throws JAXBException, IOException, SAXException, ParserConfigurationException,
                    TransformerException, ParseException, SQLException {
        ...
        Terrein real =
                run.query(
                        "SELECT * FROM top250nl.terrein WHERE identificatie=?",
                        handler,
                        "NL.TOP250NL.16R09-0000064539");
        assertNotNull(real);
    }

    @Test
    public void spoorbaanDePunt()
            throws JAXBException, IOException, SAXException, ParserConfigurationException,
                    TransformerException, ParseException, SQLException {
        ...
        Terrein real =
                run.query(
                        "SELECT * FROM top10nl.spoorbaandeel WHERE identificatie=?",
                        handler,
                        "NL.TOP10NL.105531727");
        assertNotNull(real);
    }
}
