...
class ResumingInputStreamTest {
  ...
  void normalStream() throws IOException {
    ResumingInputStream is =
        new ResumingInputStream((position, totalRetries, causeForRetry) -> toStream("test"));
  ...
  void readSingleBytes() throws IOException {
    // Test the abstract read() method that must be overridden is not usually called in favor of
    // reading
  ...
  void delegateCloseCalled() throws IOException {
    ...
    ResumingInputStream is =
        new ResumingInputStream(
            (position, totalRetries, causeForRetry) ->
                new ProxyInputStream(toStream("test")) {
                  @Override
  ...
  void testMaxTriesExceeded() throws IOException {
    ResumingInputStream is =
        new ResumingInputStream((position, totalRetries, causeForRetry) -> new BrokenInputStream());
