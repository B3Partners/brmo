...
public abstract class SchemaSQLMapper {
    ...
    public String getColumnNameForObjectType(ObjectType objectType, String attributeName) {
        ...
        if (i != -1) {
            attributeName =
                    new StringBuilder(attributeName)
                            .replace(i, i + tableNameLower.length(), "")
                            .toString();
        }
    ...
    public void printSchema(
        ...
        String createTable =
                tableNamesObjectTypes.values().stream()
                        .flatMap(
                                objectType ->
                                        getCreateTableStatements(objectType, dialect, tablePrefix))
                        ...
                        .flatMap(
                                objectType ->
                                        getCreateGeometryMetadataStatements(
                                                objectType, dialect, tablePrefix))
        ...
        System.out.printf(
                "-- %s %s\n\n",
                getBundleString("schema.primary_keys"),
                getBundleString("schema.after_initial_load"));
        String primaryKeys =
                tableNamesObjectTypes.values().stream()
                        .flatMap(
                                objectType ->
        ...
        System.out.printf(
                "-- %s %s\n\n",
                getBundleString("schema.geometry_indexes"),
                getBundleString("schema.after_initial_load"));
        String geometryIndexes =
                tableNamesObjectTypes.values().stream()
                        .flatMap(
                                objectType ->
                                        getCreateGeometryIndexStatements(
                                                objectType, dialect, tablePrefix))
                        .collect(Collectors.joining(";\n"));
                        ...
                        .map(
                                column -> {
                                    ...
                                    if (column instanceof ForeignKeyAttributeMapping) {
                                        ForeignKeyAttributeMapping fkMapping =
                                                (ForeignKeyAttributeMapping) column;
                                        referencing =
                                                " references "
    ...
    public int getMaxTableLength() {
        return objectTypeNameToTableName.values().stream()
                .map(String::length)
