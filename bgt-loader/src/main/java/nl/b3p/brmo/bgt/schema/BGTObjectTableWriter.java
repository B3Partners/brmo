...
public class BGTObjectTableWriter extends ObjectTableWriter {
    ...
    private void deletePreviousVersion(BGTObject object) throws Exception {
        ...
        String tableName =
                getSchemaSQLMapper().getTableNameForObjectType(objectType, getTablePrefix());
        ...
        if (!deleteBatches.containsKey(objectType)) {
            String sql =
                    "delete from "
                            ...
                            + " where "
                            + getSchemaSQLMapper()
                                    .getColumnNameForObjectType(objectType, idAttributeName)
                            + " = ?";
            deleteBatches.put(
                    objectType,
        ...
        for (ObjectType oneToManyObjectType : objectType.getOneToManyAttributeObjectTypes()) {
            if (!deleteBatches.containsKey(oneToManyObjectType)) {
                String tableNameNoPrefix =
                        getSchemaSQLMapper().getTableNameForObjectType(objectType, "");
                String sql =
                        "delete from "
                                + getSchemaSQLMapper()
                                        .getTableNameForObjectType(
                                                oneToManyObjectType, getTablePrefix())
                                + " where "
                                + getSchemaSQLMapper()
                                        .getColumnNameForObjectType(
                                                oneToManyObjectType,
                                                tableNameNoPrefix + idAttributeName)
                                + " = ?";
                deleteBatches.put(
                        oneToManyObjectType,
                        new PreparedStatementQueryBatch(getConnection(), sql, getBatchSize()));
            ...
            executed =
                    executed
                            | deleteBatch.addBatch(
                                    new Object[] {object.getMutatiePreviousVersionGmlId()});
        }
