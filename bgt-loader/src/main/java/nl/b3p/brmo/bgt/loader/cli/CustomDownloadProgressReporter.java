        ...
        if (progress.getStatusApiCalls() == 0 && progress.getDownloadRequestId() != null) {
            log.debug(
                    getMessageFormattedString(
                            "download.request_id", progress.getDownloadRequestId()));
            ...
            if (!printProgress) {
                        ...
                        log.info(
                                getMessageFormattedString(
                                        ...
                                        String.format(
                                                "%s (%2d%%)",
                                                status,
                                                progress.getStatusResponse().getProgress())));
                        lastProgress = Instant.now();
                    }
                }
            } else {
                if (status == PENDING) {
                    ...
                    log.debug(
                            getMessageFormattedString(
                                    "download.extract_ready", progress.getTimeSinceStart()));
